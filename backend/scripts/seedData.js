...
async function seedDatabase() {
  try {
    ...
    // Only seed if no codes exist (safe to re-run)
    const existingCount = await DiscountCode.countDocuments();
    if (existingCount === 0) {
      await DiscountCode.insertMany([
        {
          ...
          value: 20,
          description: 'Standard 20% off — main promo code',
          isActive: true,
          usageLimit: null,
          minimumOrderValue: 0
        },
        ...
        },
        {
          ...
          discountType: 'fixed',
          value: 50,
          description: '₹50 off first order (min ₹299)',
          isActive: true,
          usageLimit: null,
          minimumOrderValue: 299
        }
      ]);
      console.log('✅ Seeded 3 default discount codes (GROOVY20, WELCOME10, FIRSTORDER)');
    } else {
      console.log(`ℹ️  Skipped discount codes — ${existingCount} already exist in DB`);
    }
